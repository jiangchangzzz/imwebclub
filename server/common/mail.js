...
exports.sendNewTopicToAdmin = function (data) {
  var subjectTpl = '<%= user.name || user.loginname %>' +
    '发布了新文章<%=topic.title%>';
  var htmlTpl = [
    '<p><%= me.name || me.loginname %> 管理员，您好:</p>',
    '<p>',
    '<%= user.name || user.loginname %> 发布了新文章：',
    '<a href="<%=siteUrl%>/topic/<%=topic._id%>"><%=topic.title%></a>',
    '</p>',
 ...
 * 关注的用户发布新文章，邮件通知关注者
 */
exports.sendNewTopicToFollowers = function (data) {
  var subjectTpl = '<%= user.name || user.loginname %>' +
    '发布了新文章<%=topic.title%>';
  var htmlTpl = [
    '<p><%= me.name || me.loginname %> 您好:</p>',
    '<p>',
    '您关注的 <%= user.name || user.loginname %> 发布了新文章：',
    '<a href="<%=siteUrl%>/topic/<%=topic._id%>"><%=topic.title%></a>',
    '</p>',
    '<p>快去围观吧~~</p>',
    MAIL_FOOT
  ...
  var followers = data.user.follower;
  User.getUsersByIds(followers, function (err, followerUsers) {
    if (!err) {
